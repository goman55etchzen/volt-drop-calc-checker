...
export function useReversedCallc(
  ...
  const currentSystem = computed(() => {
    return SYSTEM_DEFINITIONS.find((s) => s.id === selectedSystemId.value) || SYSTEM_DEFINITIONS[0];
  });
  ...
  const calculationIssues = computed<CalculationIssue[]>(() => {
    ...
    if (loadType.value === 'motor') {
      ...
      issues.push({
        ...
        message: 'リアクタンス成分を無視し、純抵抗として電圧降下を算出しています。',
      });
    }
  ...
  const hasError = computed(() => calculationIssues.value.some((issue) => issue.level === 'error'));
  ...
  const availableWires = computed<AvailableWireResult[]>(() => {
    ...
    return CABLE_SPECS.map((spec) => {
      const z = ignorePowerFactor.value ? spec.r : (spec.r * cosTheta + spec.x * sinTheta);
      ...
      return {
        ...
        isOkForLoad: effectiveMaxAmp >= I && allowAmpereByHeat >= requiredWireAmp,
      };
    });
  });
